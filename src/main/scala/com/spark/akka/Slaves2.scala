...
/**
  ...
  */
class Slaves2(slaveId: String, memory: Int, cores: Int) extends Actor{
...
object Slaves2{
    val host = AkkaConfig.getProperty(ConfigManager.AKKA_SLAVE2_HOST)
    val port = AkkaConfig.getInt(ConfigManager.AKKA_SLAVE2_PORT)
    val masterName = AkkaConfig.getProperty(ConfigManager.AKKA_SLAVE2_NAME)
    ...
    def main(args: Array[String]): Unit = {
        ...
        val slaveActorSystem = ActorSystem("slave2", config)

        // 2、通过actorSystem来创建 slave2 actor
        val slave2: ActorRef = slaveActorSystem.actorOf(Props(new Slaves2(slaveId, 24345, 5)), masterName)

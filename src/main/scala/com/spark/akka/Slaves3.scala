...
/**
  ...
  */
class Slaves3(slaveId: String, memory: Int, cores: Int) extends Actor{
...
object Slaves3{
    val host = AkkaConfig.getProperty(ConfigManager.AKKA_SLAVE3_HOST)
    val port = AkkaConfig.getInt(ConfigManager.AKKA_SLAVE3_PORT)
    val masterName = AkkaConfig.getProperty(ConfigManager.AKKA_SLAVE3_NAME)
    ...
    def main(args: Array[String]): Unit = {
        ...
        val slaveActorSystem = ActorSystem("slave3", config)

        // 2、通过actorSystem来创建 slave3 actor
        val slave3: ActorRef = slaveActorSystem.actorOf(Props(new Slaves3(slaveId, 42372, 5)), masterName)
